---
title: "Raf_OA_Osteoarthritis-Prebiotic-Multiomics_Human_data_Rmarkdown"
author: "Weilan Wang"
date: "2025-02-04"
...
Raf_OA_MetaPhlan4_sig_species_MaAslin2_Prebiotic_figure <- ggplot(Raf_OA_MetaPhlan4_sig_species_MaAslin2_Prebiotic) +
  ...
  labs(x = "MetaPhlan4 Species", y = "β coefficient") +
  coord_flip() +  
  mytheme4 
...
sum_per_sample <- Raf_OA_Bile_acid_pathway_species_long_with_phenotypes2 %>%
  group_by(Genes, SampleID, Group, Genus) %>%
  dplyr::summarize(Sum_BA_Abundance = sum(BA_Abundance, na.rm = TRUE), .groups = 'drop')

average_per_group <- sum_per_sample %>%
  group_by(Genes, Group, Genus) %>%
  dplyr::summarize(Average_Sum_BA_Abundance = mean(Sum_BA_Abundance, na.rm = TRUE), .groups = 'drop')
...
Anaerostipes_hadrus_butyrate_lme <- lmer(PWY.5676..acetyl.CoA.fermentation.to.butanoate.II.g__Anaerostipes.s__Anaerostipes_hadrus ~ Diets * Months + (1 | Participant), data = Raf_OA_HuMaNA3_Butyrate_pathway_species_with_phenotypes, REML = FALSE)
...
Raf_OA_HuMaNA3_Met_pathway_with_phenotypes <- Raf_OA_HuMaNA3_Met_pathway_with_phenotypes %>%
   pivot_longer(
     cols = HOMOSER.METSYN.PWY..L.methionine.biosynthesis.I: PWY.6151..S.adenosyl.L.methionine.salvage.I,
     names_to = "Met_Pathways",
     values_to = "MetPathway_Abundance")
